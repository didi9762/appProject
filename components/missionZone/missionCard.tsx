    ...
    >
      {info.sender ? (
        <PricingCard
          ...
          info={[`from:${info.sender}`, `to: ${info.address}`,`save to:${info.save}`]}
          infoStyle={{ fontSize: 25 }}
          button={
            info.open
              ? { title: "Take It" }
              :saveForUser? { title: "You hold the mission" }
              :{title:"Mission On Hold"}
          }
          onButtonPress={
            info.open
              ? handleSave
              : () => {
                  Alert.alert(
                    "sorry",
                    "Mission already taken try later if change"
                  );
                }
          }
        />
